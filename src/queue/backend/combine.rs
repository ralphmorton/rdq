...
#[derive(Clone)]
pub struct Combine<I1, I2, B1: Backend<I1>, B2: Backend<I2>> {
...
impl<I1, I2, B1: Backend<I1>, B2: Backend<I2>> Combine<I1, I2, B1, B2> {
    pub fn new(backend1: B1, backend2: B2, dequeue_strategy: DequeueStrategy) -> Self {
        Self {
    ...
    async fn dequeue_precedence(
        ...
        let items: Vec<Either<I1, I2>> = self
            .backend1
            .dequeue(n, None)
            .await?
            .into_iter()
...
impl<I1: Send + Sync, I2: Send + Sync, B1: Backend<I1> + Send + Sync, B2: Backend<I2> + Send + Sync>
    Backend<Either<I1, I2>> for Combine<I1, I2, B1, B2>
{
    async fn enqueue(&mut self, item: &Either<I1, I2>) -> Result<(), Error> {
        match item {
    ...
    async fn ack(&mut self, items: &Vec<&Either<I1, I2>>) -> Result<(), Error> {
        let i1 = items
            .into_iter()
            .filter_map(|i| Either::as_left(*i))
            .collect();
        let i2 = items
            .into_iter()
            .filter_map(|i| Either::as_right(*i))
            .collect();
...
mod tests {
    ...
    impl<I: Item + Clone + Send + Sync> Backend<I> for TestBackend<I> {
        async fn enqueue(&mut self, item: &I) -> Result<(), Error> {
            self.enqueued.lock().unwrap().push_back(item.clone());
    ...
    mod nesting {
        ...
        async fn works() {
            ...
            let b3: TestBackend<JsonItem<i32>> = TestBackend::new();
            let b2_b3 = Combine::new(b2.clone(), b3.clone(), DequeueStrategy::Precedence);
            let mut c = Combine::new(b1.clone(), b2_b3, DequeueStrategy::Precedence);
            ...
            // Enqueue into `b2`
            c.enqueue(&Either::Right(Either::Left(JsonItem::new(4))))
                .await
                .unwrap();

            // Enqueue into `b3`
            c.enqueue(&Either::Right(Either::Right(JsonItem::new(5))))
                .await
                .unwrap();

            let enqueued_b1: Vec<JsonItem<i32>> = b1.get_enqueued().into_iter().collect();
            let expected_b1 = vec![JsonItem::new(1), JsonItem::new(2), JsonItem::new(3)];
            assert_eq!(enqueued_b1, expected_b1);

            let enqueued_b2: Vec<JsonItem<i32>> = b2.get_enqueued().into_iter().collect();
            let expected_b2 = vec![JsonItem::new(4)];
            assert_eq!(enqueued_b2, expected_b2);

            let enqueued_b3: Vec<JsonItem<i32>> = b3.get_enqueued().into_iter().collect();
            let expected_b3 = vec![JsonItem::new(5)];
            assert_eq!(enqueued_b3, expected_b3);

            // Dequeues first from `b1`.
            ...
            // Dequeues from `b1` again since there are remaining items and
            // it is highest precedence.
            let dequeued = c.dequeue(1, None).await.unwrap();
            let expected = vec![Either::Left(JsonItem::new(3))];
            assert_eq!(dequeued, expected);

            // `b1` is drained, next dequeue will pull from `b2`.
            let dequeued = c.dequeue(1, None).await.unwrap();
            let expected = vec![Either::Right(Either::Left(JsonItem::new(4)))];
            assert_eq!(dequeued, expected);

            // `b1` and `b2` are drained, next dequeue will pull from `b3`.
            let dequeued = c.dequeue(1, None).await.unwrap();
            let expected = vec![Either::Right(Either::Right(JsonItem::new(5)))];
            assert_eq!(dequeued, expected);
    ...
    mod round_robin {
        ...
        async fn dequeues_round_robin() {
            ...
            let enqueued_b1: Vec<JsonItem<i32>> = b1.get_enqueued().into_iter().collect();
            let expected_b1 = vec![JsonItem::new(1), JsonItem::new(3), JsonItem::new(5)];
            assert_eq!(enqueued_b1, expected_b1);

            let enqueued_b2: Vec<JsonItem<i32>> = b2.get_enqueued().into_iter().collect();
            let expected_b2 = vec![JsonItem::new(2), JsonItem::new(4)];
            assert_eq!(enqueued_b2, expected_b2);
            ...
            let dequeued = c.dequeue(2, None).await.unwrap();
            let expected = vec![Either::Left(JsonItem::new(5))];
            assert_eq!(dequeued, expected);
